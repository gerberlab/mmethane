...
def plot_metab_original_dists(met_ids, dmat, rule_detect_ls, dir):
    for i in range(len(rule_detect_ls)):
        if len(met_ids[i]) == 0:
            continue
        # try:
        fig, ax = plt.subplots()
        try:
            met_dists = np.delete(np.concatenate([dmat[met_ids[i], k].flatten() for k in met_ids[i]]), 0)
        except:
            met_dists = np.delete(np.array([dmat[met_ids[i], k].flatten() for k in met_ids[i]]), 0)
        ex = np.array(list(set(np.arange(dmat.shape[0])) - set(met_ids[i])))
        if len(ex) == 0:
            return
        try:
            out_dists = np.delete(np.concatenate([dmat[ex, k].flatten() for k in ex]), 0)
        except:
            out_dists = np.delete(np.array([dmat[ex, k].flatten() for k in ex]), 0)
        # except:
        #     continue
        # out_dists = np.delete(np.concatenate([dmat[ex, k].flatten() for k in ex]),0)
        all_dists = dmat.flatten()
        _, bins, _ = ax.hist(all_dists.flatten(), bins=20, alpha=0)
        ax.hist(out_dists.flatten(), bins=bins, alpha=0.2, label='Dists b/w out-group metabolites', density=True)
        ax.hist(met_dists.flatten(), bins=bins, alpha=1, label='Dists b/w metabolites in detector', density=True)
        # ax.axvline(x=np.exp(radii[i]))
        ax.set_title('Rule {0} Detector {1}'.format(rule_detect_ls[i][0], rule_detect_ls[i][1]))
        ax.legend()
        fig.savefig(dir + '/rule_{0}_detector_{1}/actual_dists.png'.format(rule_detect_ls[i][0], rule_detect_ls[i][1]))
        plt.close(fig)
    # mds_df = pd.DataFrame(dmat, index = all_metabs)
