...
def plot_data_heatmap(data_0: pd.DataFrame, data_1: pd.DataFrame, threshold, cmap, ylabels=True, class_labels=None,
    ...
    ax[0].tick_params(axis='y', labelsize=f)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--path", type=str, default='logs/my_first_run/seed_0/')
    ...
    for ir, r in enumerate(det_params.keys()):
        ...
        heights.append(1)
        # fig, ax = plt.subplots(len(det_params[r].keys()), 3, sharex=True, sharey=True,
        ...
        for d in det_params[r].keys():
            ...
            if det_params[r][d]['dtype'] == 'metabs':
                ...
                for im, li in zip(images_left, links):
                    if li != '':
                        ...
                        new_im = im
                images_left = '<div class="column">' + "".join(new_images_left) + '</div>'
            else:
                cmap = cmap_taxa
                agg = 'sum'
                tmp = 1.75
            ...
            fig_data, _, max_labels = plot_data_heatmap(dtmp['data_0'], dtmp['data_1'], dtmp['threshold'], cmap=cmap,
                                                        class_labels=[args.outcome_negative_value,
                                                                      args.outcome_positive_value],
        ...
        fig_det_act, _ = plot_activation_heatmap(rule_activators[r], Y, cmap_act, ylabel=f"Rule {rct} Activation",
                                                 class_labels=[args.outcome_negative_value,
                                                               args.outcome_positive_value],
                                                 max_ylabels=max(ml))
...
img {
    ...
    border-width: 2px;
    border-color: #ccc;
    max-width: 100%; /* Make sure images fit the column width */
